...
def check_V(state, face, color):
    ...
    if count1 >= 2:
        if state[face]["edges"][0] == color and state[face]["edges"][1] == color:
            if state["front"]["edges"][1] == "green" and state["right"]["edges"][1] == "blue":
                return True
            else:
                return False
        if state[face]["edges"][0] == color and state[face]["edges"][2] == color:
            if state["front"]["edges"][1] == "green" and state["left"]["edges"][1] == "red":
                return True
            else:
                return False
        if state[face]["edges"][1] == color and state[face]["edges"][2] == color:
            if state["right"]["edges"][1] == "blue" and state["left"]["edges"][1] == "red":
                return True
            else:
                return False
    else:
...
def bfs_solve(initial_state, target_face='bottom', target_color='yellow'):
    ...
    while queue:
        ...
        if check_V(current_state, target_face, target_color):
            print("解决方案步骤:", path)
            print(current_state)
            return path
